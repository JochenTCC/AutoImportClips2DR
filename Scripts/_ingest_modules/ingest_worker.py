    ...
    if format_mode != "NONE":
        ...
        for day_str in sorted_days:
            ...
            if not clips_to_import:
                log_callback(f"   -> [INFO] Keine neuen Medien zum Importieren für {sub_folder_name} gefunden (oder auf Blacklist).")
                continue
        ...
        if not clips_to_import:
            log_callback(f"   -> [INFO] Keine neuen Medien zum Importieren für flachen Ordner gefunden (oder auf Blacklist).")
            return  
...
def run_ingest_process(format_mode, use_h265, log_callback, create_pancakes, camera_colors=None, base_drx_dir="", camera_mappings=None, base_target_dir="", base_proxy_dir=""):
    ...
    try:
        ...
        if trash_filepaths:
            log_callback(f"[INFO] {len(trash_filepaths)} Clips auf der Blacklist (_Ausschuss) erkannt. Diese werden beim Ingest ignoriert.")
        ...
        if not detected_sd_cards:
            ...
            # WICHTIG: Auch hier das [FEHLER]-Flag werfen, damit die GUI nicht einfriert!
            return
        ...
        for label, source_drive in detected_sd_cards.items():
            if source_drive and not has_media_files(source_drive):
                log_callback(f"   -> Laufwerk {source_drive} ({label}) übersprungen: Keine unterstützten Mediendateien gefunden.")
                continue
        ...
        # PROXIES: Rendern, falls neue RAW-Clips in der Queue gelandet sind
        if raw_queue:
        ...
        # KORREKTUR: Dieses Flag MUSS unkonditionell am Ende stehen, damit der GUI-Monitor stoppt!
...
def run_cleanup_process(log_callback):
    ...
    try:
        ...
        for clip in all_footage_clips:
            ...
            if usage == "" or usage == "0" or usage == 0 or usage is None:
        ...
        log_callback(f"[CLEANUP] {len(unused_clips)} ungenutzte(n) Clip(s) identifiziert. Verschiebe nach _Ausschuss...")
        
        success = media_pool.MoveClips(unused_clips, trash_bin)
        if success:
            log_callback(f"[FERTIG] {len(unused_clips)} Clip(s) erfolgreich in den Ordner '_Ausschuss' verschoben und für zukünftige Ingests blockiert!")
        else:
            log_callback("[FEHLER] Clips konnten von der Resolve API nicht verschoben werden.")
